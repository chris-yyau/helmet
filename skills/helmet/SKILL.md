  ...
  release:
    ...
    steps:
      ...
      - name: Release
        ...
        run: npx -y -p semantic-release@25.0.7 -p @semantic-release/github@12.0.9 semantic-release
```
...
jobs:
    ...
    steps:
      ...
      - name: Detect security-relevant changes
        ...
        run: |
          ...
          grep_status=0
          grep -qE '\.(sh|js|py|yml|yaml|tf)$|\.github/|package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|go\.sum|requirements.*\.txt|Dockerfile|pyproject\.toml|uv\.lock|Cargo\.lock|Cargo\.toml|Package\.resolved|Package\.swift' <<< "$changed_files" || grep_status=$?
          case "$grep_status" in
  ...
  semgrep:
    ...
    steps:
      ...
      - name: Install semgrep
        run: pip install --quiet 'semgrep==1.170.0'  # Pin version — unpinned is a supply chain risk; lockstep: see issue #67
  ...
  zizmor:
    ...
    steps:
      ...
      - name: Install zizmor
        run: pip install --quiet 'zizmor==1.27.0'  # Pin version — unpinned is a supply chain risk; lockstep: see issue #67
...
jobs:
  trivy-scheduled:
    ...
    steps:
      ...
      #
      # `output:` writes to RUNNER_TEMP, never the workspace: a repo that happens to
      # contain a checked-in `trivy-report.txt` would otherwise leave stale content in
      # place when trivy dies before writing, and the emptiness check below would read
      # that repo file and file it as CVE findings.
      - name: Scan dependencies
        id: scan
      ...
      # trivy writes the table to a file, not stdout — without this the run log holds no
      # findings at all, and the truncation notice in the issue body would point at
      # nothing. Echo the full table so "see the run log" is a true statement.
      ...
      - name: Open tracking issue
        ...
        run: |
          ...
          # all. Clip a COPY for the body — the full table is in the run log, printed by
          # the step above. Never clip $REPORT itself; the log step already ran, but a
          # destructive edit here would strand any later reader of the file.
          max_report=60000
          body_report="$RUNNER_TEMP/report-for-body.txt"
          if [ "$(wc -c < "$REPORT")" -gt "$max_report" ]; then
            head -c "$max_report" "$REPORT" > "$body_report"
            printf '\n... [truncated at %s chars — full table in the run log: %s]\n' \
              "$max_report" "$RUN_URL" >> "$body_report"
          else
            cp "$REPORT" "$body_report"
          fi
      ...
      - name: Close tracking issue when clean
        ...
        run: |
          ...
          #
          # URL-encode the ref before splicing it into the API path. A git ref may legally
          # contain `%`, `#`, `{`, `}` — every one hazardous in a `gh api` path: `#` is a URL
          # fragment delimiter, `{owner}`/`{repo}`/`{branch}` are placeholders gh expands, and
          # `%` is the encoding escape. Any of them would silently retarget the request at a
          # DIFFERENT ref, so the close-on-green path could close a still-vulnerable ref's
          # issue (fail-open on the security signal). This four-char set is complete, not a
          # patch: git's own ref rules forbid the other URL-significant chars (space, `?`,
          # `~^:*[\`), so they can never reach here, and remaining ref-legal chars (`@ & = ; ]`
          # etc.) are literal inside a path segment. Encode `%` FIRST so it never double-
          # encodes the `%23`/`%7B`/`%7D` the later passes introduce; leave `/` intact — the
          # commits API takes a full slash-bearing ref (`refs/heads/main`).
          ref_path=$REF
          ref_path=${ref_path//%/%25}
          ref_path=${ref_path//\#/%23}
          ref_path=${ref_path//\{/%7B}
          ref_path=${ref_path//\}/%7D}
          head_sha=$(gh api "repos/$REPO/commits/$ref_path" --jq '.sha')
